...
void game::window_loop() {
  ...
  while (window->isOpen() && state_run) {
    ...
    // Replaced by simply running a detached thread which wraps the task in a future and waits for exceptions
  ...
  socket->instruction = socket_t::tc_stop;
  state_run = false;
}
...
void game::queue_background_task(Voidfun callback) {
  // static int tid = 0;
  ...
  thread t([this, callback] () {
    future<void> f = async(launch::async, callback);
    try {
      f.wait();
