...
**** solar evolution
** client game
*** change to selector : public data_thingy
*** incrementing command
    consider what happens when multiple commands with same source are
    incremented at limit. Currently, the first feasible commands in
    the list will be incremented.

    write x_selector::get_quantity();
