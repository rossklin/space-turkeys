* current work
** path search
   For some reason, the same point gets extracted from the frontier
   multiple times, even though it seems like it is not
   reinserted. Also, path search fails.

   Consider just recursively passing outer sight-point of intersecting
   polygons instead.
** balancing
