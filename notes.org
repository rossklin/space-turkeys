...
**** add to command: action (from above options)
**** add command::action to command::operator ==
**** add queue_level to command
**** replace entity_at with entities_at
**** replace right click event handler with target option gui
*** clickable minimap
