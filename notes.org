...
*** [?] "space_status: used more than space" reached after developing facility
** mechanical errors
*** Ships validate their action using target information that the player can't see
** [~] system
*** [J?] Ships collide while travelling in fleet
    Check free space on either side
*** big ships are too clumsy and collide when courses intersect
*** population adapts too quickly to circumstances e.g. space
*** solars run out of space after building some stuff
    Make facilities not use more space per level. Add attribute
    "provides space" and "provides water".
...
*** client allows multiple development choices - server does not
    Redesign: chose development in advance like everything else
...
* [!] system design ideas
** add a list of animations to send to client rather than add explosion on ship removal
** add expansion limitations e.g. nr of fleets and solar distance penalty
** add a battle-log to send to client
...
*** add "build" button on facilities in solar_gui
*** command selectors should update their position on each reload_data
