...
* git pending
  Made command gui sort ships by type. Added a minimap and fixed text
  scale and position. Implemented research pooling on server
  side. Fixed a bug where fleets were incorrectly idled and waypoints
  removed.
* fixed
