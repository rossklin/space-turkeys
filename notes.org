...
** In engage mode ships chose path to fleet center, issue with fleet->stats.enemies?
** Ships seem to engage when in evasive mode, perhaps when they are too far from enemy to detect threat?
** Heavy lag in client with many enemies
   Look at moving parts of reload_data to load_frames
** Removing ships from command does not remove downstream ships
...
** [impl] Waypoint becomes impossible to select after first select or movement
