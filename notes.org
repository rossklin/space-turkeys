...
* basic goals
** server game
*** neutral planets should start with less industry
*** ability to build defense
*** remove waypoints with neither incoming nor outgoing commands
...
** structure
*** remove solar::development as no longer nessecary
* bugs
** fleets follow targets that leave sight
** fleets follow last assigned in split
* issues
** entity_selector does not need attribute allocated (sum over commands)
...
* git pending
** renamed Makefile -> Makefile.linux
** set up for autotools - add configure to git
* fixed
