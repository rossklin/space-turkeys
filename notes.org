...
** balancing
*** early
    +fighter vs scout (unchanged)
**** log
    +fighter vs scout
**** log
    +fighter vs scout
**** log
    -scout vs light: -50% scout damage
*** mid: ionized hulls, plasma weapons, shield technology
    ++battleship vs corsair (unchanged)
    ++fighter vs battleship (overshot)
**** log
    battleship: facility level -1, accuracy +40%

    ++battleship vs corsair
    +++battleship vs fighter
**** log
    +++battleship vs corsair
**** log
    +battleship vs light, med
    +corsair vs light

    fighter: +100% hp, -60% damage
    ionized hulls: -67% effect
    shield: -75% effect
*** late: micro torpedos
    ++voyager vs [destroyer, corsair] (unchanged)
    --voyager vs fighter (new)
    ++battleship vs [destroyer, corsair, fighter] (unchanged)
    ++corsair vs fighter (new)
    ++fighter vs destroyer (improved)
**** log
    ++fighter vs voyager: micro torpedos +40% const bonus
    ++voyager vs [destroyer, battleship, corsair]: +33% damage
    +++battleship vs [corsair, fighter]: +25% shield
    ---destroyer vs [battleship, fighter]: -50% accuracy
**** log
    +++voyager vs [battleship, destroyer, corsair]
    +++battleship vs [corsair, destroyer, figther]
    ---destroyer vs [battleship, voyager, fighter]
    +destroyer vs corsair
**** log
    +voager vs heavy, med: +50% shield
    -destroyer vs ALL: +67% load time
    +battleship vs ALL: +3 shield
    +corsair vs light
*** late SPLASH
    -destroyer vs battleship (improved)
    --destroyer vs fighter (unchanged)
**** log
    ---destroyer vs battleship
    --destroyer vs fighter
    --battleship vs fighter
*** late HM
    ++corsair vs fighter (overshot)
**** log
    ++fighter vs corsair: -20% damage
    -fighter vs destroyer
    ---fighter vs battleship
*** late HMSPLASH
    --fighter vs destroyer (overshot)
    ++fighter vs battleship (worsened)
**** log
    +fighter vs destroyer
    +fighter vs battleship
**** log
    +fighter vs [corsair, voyager, battleship]: splash -33% damage
**** log
    +corsair vs light
** play testing
*** gui is too small on large screen, set fixed video mode or scale text to relative size
*** slower ships left behind
...
*** command gui is terribly laggy
*** [J?] fleet must "activate" when first ships reach target
