...
** client game
*** have an entity_selector -> queue_level
    increment whenever an entity is selected, so that next time an
    entity below it can be selected instead
...
* bugs
** undefined behaviour for circular waypoint graphs
   prevent creation of circular waypoint graphs
* issues
** ships bombard and land in order of fleet, unfair
** server sends fleets with missing target to client
   when the target is a fleet which has dissappeared after the source
   was updated
** ships in fleets can go at different speeds
