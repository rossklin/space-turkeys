...
** GUI
*** Research component
  Too low contrast on basic theme
  Text in description is badly rendered
  Rename button "Select" or something
  Hovering an option takes ~100 ms

** Game client state logic
  No effect pressing done while mid simulation
  - because load_frames waits until frames are needed (why?), pre_step will not start until all frames are generated

  Stuck in "loading" after pressing done in simulation (one client pressed before all frames loaded..?)
  Server is sending "standby", so perhaps server gets stuck in check protocol if client requests next step?

  Probably restructure so "check_protocol" is always running, in the context of the "game", and fetches the appropriate 
  handler callback based on the query, and updates the client state to track which game phase they are on?

** Game physics
  Collisions make ships bubble around, guarantee relative outgoing speed is always lower than incoming speed

  Fleet positions must be updated correctly, left behind when ships move to attack

** [done] Removing ships from command does not remove downstream ships
  Also command gui for downstream commands shows original ships
** [done] Waypoint becomes impossible to select after first select or movement
** [test] prune_path not working correctly
  Verify

  Misses initial point sometimes.
  Or maybe just turns out that way after fleet moves?
** [done] Consistent network error pressing done in simulation before other client pressed commit
   Confirmed error in sf::Packet& st3::operator>>(sf::Packet& packet, game_base_data& g)

   The second client to click done gets a packet with 1 frames
   generated and a confirm, containing idc, players and settings. The
   rest of the packet is corrupted.

   Some concurrency thing with "done" callback and use of socket data buffer?
...
** Server slow in large battle
   Testing new grid::tree   

   Mainly caused in grid::tree::search, by iterating the index and
   looking up the metadata. 

   https://en.wikipedia.org/wiki/Locality-sensitive_hashing
** Simulation progress bar resets to 0 when data runs out or when paused?
** Fleet evasion paths are a bit quirky
   Look at scalings and bandwidths in analyze_enemies

   For example:
   Heading towards enemy fleet when only spotted a few ships
   Heading to other side of enemy fleet when standing on home solar
...
* Development
** New grid::tree
*** Update to using int grids per class instead of combid grid to get fixed memory
*** Fix optimization issues ie trig functions
*** Make sure entities can't leave grid
*** Update or remove extend_universe
*** [done] Remove entity_grid from game_base_data so it isn't rebuilt in client deserializer
    
**** Update entity_grid after calls to add_entity and remove_entity
     client::deserialize: can f be a client_data_frame?
     client::game: add method register_entity which calls grid::tree::insert, replace calls to add_entity
     game_data::register_entity: call grid::tree::insert
     client::game and game_data: deregister_entity: call grid::tree::remove
*** Eventually handle terrain in grid
** Actions (at least deploy and go to) need a "face direction" property
...
** Look over descriptions in all choice UIs
** Event log: color coded cards, click -> go to map location
* Ideas
...
** Different mechanics for "flagships", medium size ships and fighters
   Perhaps fighters should not collide with other ships except own fleet?
   Perhaps treat fleet and ship count limits differently?
   Perhaps allow players to restructure fleets with flagships?   
...
** [done] In engage mode ships chose path to fleet center, issue with fleet->stats.enemies?
** [test] Fleets should stop moving along path in engage or evade mode
** [done] Heavy lag in client with many enemies
   Look at moving parts of reload_data to load_frames
** [done] When fleeing or engaging, ships drive straight into terrain
** [test] Owned ships left as shadows when dead in some cases
** [fixed] Arrival to waypoint, fleet idles but next command removed instead of triggered
** [test] Evasive fleet policy does not work (well enough)
...
** [done] Alternative movement approach3

   Maybe ships who have hpos in front of fleet should always use trail instead of terrain?
   [impl] Always use most forward path node?
   [impl] Update neighbour influence factor
   [impl] Check ship pop path logic

   [impl] Only push towards fleet center if in sight
   [impl] Visualize ship private paths in client to debug why some paths are still weird
...
** [impl] Fleet heading uninit or set to origin after auto commit
   Check how fleet heading is initialized in update_heading after
   creation in fleet::give_commands
** [done] Replace collisions with repulsing potential
** [done] Movement issues
...
** [done] Update fleet position in give_commands
** [done] Client should delete all fleets in reload_data?
