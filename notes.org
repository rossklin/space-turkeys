* current work
** test run fleet intelligence on more than one ship
** research completes too fast
** fleets seem to idle when reaching solar before all ships land
** sector boosts apply exponentially with level and should be lower
** (see above) movement is too slow in relation to development
** protect from spacebar immediately after simulation is finished
** mining is too efficient
** default ship assignment for special action should be one compatible ship
** log panel grows sideways off screen, info panel follows
** state of water, space and ecology of new solars is too random
** fleet summon doesn't seem to work very well
** trade actions can't swap fleet action since this prevents other ships in fleet from trading
** animations move too fast - bind them to frames!
** progress bar goes behind right panel
** upgrade fighters have too much health vs damage
