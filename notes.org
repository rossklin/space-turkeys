...
**** Drag map
**** Click minimap
** Component should have parameters for allowed scroll directions
** Layers should have a view connected to them (so target gui can be on game view)
** Might need a mutex for socket, since used in parallel by load_frames
** UI updates
*** Base panel
**** Hover info floating above container
     Probably caused by multiline string in label, selector::hover_info should return list
...
*** Target GUI
**** Target GUI must block scrool zoom mapping since it requires scroll in option lists
**** Target GUI option list scrolls wrong dimension
**** Target GUI should allow more space for options
**** target gui target buttons should have labels
*** Simulation interface: needs progress bar, show base panel but not commit button
...
** bugs
*** check_background_tasks should call get() to trigger exceptions
*** Font displays ugly in UI
*** Tell server quit causes network exception on client::query -> receive_packet 
